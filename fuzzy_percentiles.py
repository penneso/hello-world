...
def fill_bins(scores, bin_count):
    ...
    bins = [[k]*v for k, v in sorted(counts.items())]
...
def reduce_bins(bins, bin_count):
    ...
    while len(bins) > bin_count:
        ...
        bins[location_to_combine].extend(bins.pop(location_to_combine + 1))
...
def find_small_bin_combos(bins):
    ...
    for i, b in enumerate(bins):
        if len(b) + len(bins[i + 1]) < min_length:
            min_length = len(b) + len(bins[i + 1])
            location = i
    return location
...
if __name__ == '__main__':
    bin_count = 100#input("Number of bins:")
    filename = 'input_scores.csv'#input("Name of csv with scores:")
